...
#ifdef SOSEMANUK_ECRYPT
void
SOSEMANUK_ECRYPT_keysetup(SOSEMANUK_ECRYPT_ctx *kc, const u8 *key, u32 keysize, u32 ivsize)
#else
/* see sosemanuk.h */
void
...
#ifdef SOSEMANUK_ECRYPT
void
SOSEMANUK_ECRYPT_ivsetup(SOSEMANUK_ECRYPT_ctx *ctx, const u8 *iv)
#else
/* see sosemanuk.h */
void
...
static void
sosemanuk_internal(SOSEMANUK_ECRYPT_ctx *rc, u8 *dst)
...
/* see ecrypt-sync.h */
void
SOSEMANUK_ECRYPT_process_bytes(int action, SOSEMANUK_ECRYPT_ctx *ctx,
	const u8 *input, u8 *output, u32 msglen)
{
	(void)action;

	while (msglen > 0) {
		unsigned char tbuf[SOSEMANUK_ECRYPT_BLOCKLENGTH];
		size_t len;
...
/* see ecrypt-sync.h */
void
SOSEMANUK_ECRYPT_keystream_bytes(SOSEMANUK_ECRYPT_ctx *ctx, u8 *keystream, u32 length)
{
	while (length > 0) {
		if (length >= SOSEMANUK_ECRYPT_BLOCKLENGTH) {
			sosemanuk_internal(ctx, keystream);
			keystream += SOSEMANUK_ECRYPT_BLOCKLENGTH;
			length -= SOSEMANUK_ECRYPT_BLOCKLENGTH;
		} else {
			unsigned char tbuf[SOSEMANUK_ECRYPT_BLOCKLENGTH];
...
/* see ecrypt-sync.h */
void
SOSEMANUK_ECRYPT_process_blocks(int action, SOSEMANUK_ECRYPT_ctx *ctx,
	const u8 *input, u8 *output, u32 blocks)
{
	...
	while (blocks -- > 0) {
		...
		sosemanuk_internal(ctx, tbuf);
		xorbuf(input, tbuf, output, SOSEMANUK_ECRYPT_BLOCKLENGTH);
		input += SOSEMANUK_ECRYPT_BLOCKLENGTH;
		output += SOSEMANUK_ECRYPT_BLOCKLENGTH;
	}
}

/* see ecrypt-sync.h */
void
SOSEMANUK_ECRYPT_keystream_blocks(SOSEMANUK_ECRYPT_ctx *ctx, u8 *keystream, u32 blocks)
{
	while (blocks -- > 0) {
		sosemanuk_internal(ctx, keystream);
		keystream += SOSEMANUK_ECRYPT_BLOCKLENGTH;
...
#ifdef SOSEMANUK_ECRYPT
	SOSEMANUK_ECRYPT_init();
	SOSEMANUK_ECRYPT_keysetup(&ctx, key, key_len * 8, iv_len * 8);
	SOSEMANUK_ECRYPT_ivsetup(&ctx, iv);
#if defined SOSEMANUK_TEST_ENCRYPT_BYTES
	memset(tmp, 0, sizeof tmp);
	SOSEMANUK_ECRYPT_encrypt_bytes(&ctx, tmp, tmp, sizeof tmp);
#elif defined SOSEMANUK_TEST_DECRYPT_BYTES
	memset(tmp, 0, sizeof tmp);
	SOSEMANUK_ECRYPT_decrypt_bytes(&ctx, tmp, tmp, sizeof tmp);
#elif defined SOSEMANUK_TEST_ENCRYPT_BLOCKS
	memset(tmp, 0, sizeof tmp);
	SOSEMANUK_ECRYPT_encrypt_blocks(&ctx, tmp, tmp, 2);
#elif defined SOSEMANUK_TEST_DECRYPT_BLOCKS
	memset(tmp, 0, sizeof tmp);
	SOSEMANUK_ECRYPT_decrypt_blocks(&ctx, tmp, tmp, 2);
#elif defined SOSEMANUK_TEST_KEYSTREAM_BLOCKS
	SOSEMANUK_ECRYPT_keystream_blocks(&ctx, tmp, 2);
#else
	SOSEMANUK_ECRYPT_keystream_bytes(&ctx, tmp, sizeof tmp);
#endif
#else
