...
void HC128_setup_update(HC128_ECRYPT_ctx* ctx)  /*each time 16 steps*/
{
   u32 cc,dd;
   cc = ctx->counter1024 & 0x1ff;
   dd = (cc+16)&0x1ff;
...
void HC128_ECRYPT_ivsetup(HC128_ECRYPT_ctx* ctx, const u8* iv)
{ 
    u32 i;
	...
	for (i = 0; i < 64; i++)  HC128_setup_update(ctx);  
}
